...
Before writing any code, understand the context and commit to a clear direction.

**Understand the purpose:**
- What problem does this interface solve? Who uses it? Brand mode or product mode?
- What action should the visitor take? What is the primary CTA?
- What objections or hesitations might prevent conversion?
- What device will most users be on? (Default assumption: mobile first — 80%+ of web traffic)

**Choose a bold aesthetic direction:**
Pick a clear visual tone and execute with precision. Bold maximalism and refined minimalism both
work — the key is intentionality, not intensity. Choose from flavors like:
...
Refer to `references/design-inspiration.md` for aesthetic category guides when seeking direction.

**Plan the component tree:**
Before writing code, mentally map the component hierarchy — atoms (buttons, inputs, badges),
molecules (cards, nav items, form groups), organisms (header, hero, feature grid, footer).
This prevents spaghetti markup and ensures reusable, modular structure.

**One composition per first viewport:**
The hero area is the single most important piece of visual real estate. Commit to one strong
composition — one headline, one supporting line, one CTA, one visual anchor. No cluttered heroes.
No "everything above the fold" syndrome.

**Decide light vs. dark early.** Commit and build the token system around it. Don't switch
mid-implementation.
