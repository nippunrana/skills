...
Setting up animations, cleaning up on unmount, and avoiding context/SSR issues
in React or React-based frameworks like Next.js. For tweens and timelines see
`SKILL.md` and `references/timeline.md`; for scroll-based animation see
`references/scrolltrigger.md`; for Vue/Svelte or other frameworks see
...
- If GSAP is imported at top level, ensure the app does not execute gsap.* or ScrollTrigger.* during server render. For splitting hero vs below-the-fold animation code with `next/dynamic`, see `references/loading.md`.
