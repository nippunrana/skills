...
Pure helper functions for math, array/collection handling, unit parsing, and
value mapping — used in tween vars, ScrollTrigger/Observer callbacks, or any
JS that drives GSAP. Use alongside `SKILL.md`, `references/timeline.md`, and
`references/scrolltrigger.md` when building animations; CustomEase and other
easing utilities are in `references/plugins.md`.
