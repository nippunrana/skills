...
Using or reviewing code that uses GSAP plugins: registering plugins,
scroll-to, flip/FLIP animations, draggable elements, SVG (DrawSVG, MorphSVG,
MotionPath), text (SplitText, ScrambleText), physics, easing plugins
(CustomEase, EasePack, CustomWiggle, CustomBounce), or GSDevTools.
ScrollTrigger has its own reference file (`references/scrolltrigger.md`). For
core tweens see `SKILL.md`; for React see `references/react.md`.

## Contents

- [Licensing & Install (important)](#licensing--install-important)
- [Registering Plugins](#registering-plugins)
- [Scroll](#scroll) — ScrollToPlugin, ScrollSmoother
- [DOM / UI](#dom--ui) — Flip, Draggable, Inertia, Observer
- [Text](#text) — SplitText, ScrambleText
- [SVG](#svg) — DrawSVG, MorphSVG, MotionPath, MotionPathHelper
- [Easing](#easing) — CustomEase, EasePack, CustomWiggle, CustomBounce
- [Physics](#physics) — Physics2D, PhysicsProps
- [Development](#development) — GSDevTools
- [Other](#other) — Pixi
...
Smooth scroll wrapper (smooths native scroll). Requires ScrollTrigger (see `references/scrolltrigger.md`) and a specific DOM structure (content wrapper + smooth wrapper). Use when smooth, momentum-style scroll is needed. See GSAP docs for setup; register after ScrollTrigger. DOM structure would look like:
...
Custom easing curves (cubic-bezier or SVG path). Use when a built-in ease is not enough. Basic usage is covered in `SKILL.md`; register when using:
