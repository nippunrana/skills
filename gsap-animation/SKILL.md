...
Official GSAP guidance, consolidated from GreenSock's 8 upstream skills
...
This file alone is enough for single tweens, easing, stagger, transforms, and
responsive/reduced-motion setup. For anything else, read the matching
...
the task needs — most single-tween or basic-animation requests never need to
leave this file.

**Context:** GSAP powers **Webflow Interactions**. Code generated or run by
Webflow's interaction system is GSAP-based; when users ask about Webflow
animations or interactions not behaving as expected, the patterns in this
skill (tweens, ScrollTrigger) are relevant for debugging or customizing.
...
See `references/plugins.md` for CustomEase registration and other easing plugins (EasePack, CustomWiggle, CustomBounce).
...
- ❌ Animate layout-heavy properties (e.g. `width`, `height`, `top`, `left`) when transform aliases (`x`, `y`, `scale`, `rotation`) can achieve the same effect; prefer transforms for better performance — see `references/performance.md`.
