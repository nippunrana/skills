...
- **`contentOnly` is now the default for unsynced patterns (WP 7.0).** Do NOT add `"templateLock": "contentOnly"` to unsynced patterns — it is already active. Apply it explicitly only to synced patterns and template parts where structure must be locked. To opt out on an unsynced pattern, set `"__experimentalSettings": {"disableContentOnlyForUnsyncedPatterns": true}` on the outermost block. See `references/architecture.md → Content-Only Locking` for the full decision tree.
...
- **Viewport block visibility.** Use `metadata.blockVisibility.viewport` to show/hide blocks by device type (`"mobile"`, `"tablet"`, `"desktop"`). This removes the block from the DOM on excluded devices — do not use CSS `display: none` for this purpose. Enable with `settings.blockVisibility.viewport: true` in `theme.json`.
